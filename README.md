...
- Разработка пайплайнов для обработки видео (Video-to-Video) средствами OpenCV и Python.
    
- Интеграция диффузионных моделей (Stable Diffusion) в прикладные скрипты.
    
- Управление композицией генераций с помощью ControlNet (Canny Edge Detection).
    
- Оптимизация потребления VRAM при длительной генерации (использование `xformers` и `fp16`).
    
- Проектирование промптов (Prompt Engineering) для поддержания консистентности стиля между кадрами.
